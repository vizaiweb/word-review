...
async function loadSelectedFile(filename) {
  ...
  try {
    ...
    // ✅ CRITICAL FIX: Enable Show All button AFTER file is loaded successfully
    ...
    document.getElementById('showAllBtn').disabled = true; // Keep disabled if load fails
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById("levelConfirm").addEventListener('click', () => {
    ...
    showAllBtn.disabled = true; // Keep disabled until file load
  });

  // File selection
  document.getElementById("fileConfirm").addEventListener('click', () => {
    const file = document.getElementById("fileSelect").value;
    if (!file) { // Add validation for empty file selection
      alert('Please select a file first!');
      return;
    }
    loadSelectedFile(file);
    // Show All button will be enabled in loadSelectedFile() after success
  });
